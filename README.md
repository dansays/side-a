...
HA button ─► POST /trigger ─► app:
   ...
   → Claude writes expert "listening notes" → published to the Now Playing page
```
...
`GET /now` serves a single, mobile-formatted web page for whatever record is
currently on the platter: cover art, artist/album, a link to look it up on
AllMusic, and **listening notes** — a track-by-track guide to what musicians and
seasoned aficionados of that genre hear that a casual listener would miss
(harmonic moves, a soloist's entrance, rhythmic detail, production choices),
pinned to approximate timestamps or structural markers.

The page is **ephemeral** — it reflects only the latest scan, nothing is
persisted. It polls `GET /now/state` every few seconds and re-renders the moment
a new album is scanned, so you can leave it open on a phone by the turntable. The
album appears immediately; the listening notes are generated by Claude after
playback starts (so they never delay the audio) and fill in a few seconds later.

Open it at `APP_PUBLIC_BASE_URL/now` (the same base URL Home Assistant uses to
reach `/media`).
